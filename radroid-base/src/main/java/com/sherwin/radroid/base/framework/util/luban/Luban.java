package com.sherwin.radroid.base.framework.util.luban;
...
import com.sherwin.radroid.base.framework.util.FileUtil;
...
//        File cacheDir = context.getCacheDir();
        File cacheDir= new File(FileUtil.getInternalSDCardPath());
        if (cacheDir != null) {
            ...
            public void run() {
                ...
                try {
                    if (gear == Luban.FIRST_GEAR) {
                        file = firstCompress(mFile);
                    } else if (gear == Luban.THIRD_GEAR) {
                        file = thirdCompress(mFile);
                    }
                    ...
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            if (fFile != null) {
                                if (compressListener != null) compressListener.onSuccess(fFile);
                            } else {
                                if (compressListener != null)
                                    compressListener.onError(new RuntimeException("Compress Error Result is Null"));
                            }
                        }
                    });
